...
set title "Statistics plot for method coMDD\nIntensity normalized to maximum of 128 allplanes FT, and a=1.19493"
...
set output "allplanes_coMDD.eps"

#set fit errorvariables
#f(x)=a*x + b
#fit f(x) "allplanes_coMDD.ser" using ($6*$8 /5376722):($6*$73 /5376722) via a, b
#f_FIT_NDF=FIT_NDF
...
plot "allplanes_coMDD.stats" using (-1*$1):($71):($21):($25) title "Residual average" with yerrorbars,\
"allplanes_coMDD.stats" using (-1*$1):($47) title "Intensity proportionality a, f(x)=a*x+b",\
"allplanes_coMDD.stats" using (-1*$1):($45) title "Correlation Coefficient R^2",\
"allplanes_coMDD.stats" using (-1*$1):($39) title "Y2: Residual stdev {/Symbol s}" axis x1y2,\
"allplanes_coMDD.stats" using (-1*$1):($41) title "Y2: Covariance {/Symbol s}_{xy}" axis x1y2
